<script lang="ts">
    ...
    )
    {
        ...
        const isSelectedSiblings = selected.row === row || selected.col === col || selected.block === block;

        if (type === "bg")
        {
            if (isSelectedCell)
                return "background-color: var(--agnostic-primary-light);";
            if (isSelectedSiblings)
                return "background-color: var(--agnostic-gray-mid);";
        }
    }
</script>
            ...
            {#each { length: 3 } as _, i}
                {#each { length: 3 } as _, j}
                    ...
                    >
                        <span
                            class="digit"
                            style={getHighlightStyle(row, col, block, selected, "digit")}
                        >
                            {data.board[row][col] === 0 ? "" : data.board[row][col]}
                        </span>
                    </div>
                {/each}
            {/each}
